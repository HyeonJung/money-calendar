...
create table if not exists public.hot_deals (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint hot_deals_external_id_unique unique (external_id),
  constraint hot_deals_slug_unique unique (slug)
);

create index if not exists hot_deals_status_idx on public.hot_deals (status);
create index if not exists hot_deals_published_at_idx on public.hot_deals (published_at desc);
create index if not exists hot_deals_collected_at_idx on public.hot_deals (collected_at desc);
create index if not exists hot_deals_source_name_idx on public.hot_deals (source_name);
create index if not exists hot_deals_category_idx on public.hot_deals (category);
create index if not exists hot_deals_deal_url_idx on public.hot_deals (deal_url);

alter table public.hot_deals enable row level security;

drop policy if exists "Public read access to active hot_deals" on public.hot_deals;
create policy "Public read access to active hot_deals"
on public.hot_deals
for select
to anon, authenticated
using (status = 'active');

comment on table public.hot_deals is '공개 핫딜 소스에서 수집한 최신 할인 정보 테이블. 쓰기는 service role 전제';
comment on column public.hot_deals.external_id is '소스별 원본 식별자. 재수집 시 중복 저장을 막는 기준';
comment on column public.hot_deals.deal_url is '상품 주소. 수집 시 같은 주소가 이미 저장되어 있으면 새로 저장하지 않는다';
comment on column public.hot_deals.raw_payload is '수집 원본 구조화 데이터. 화면 노출용이 아닌 운영 추적용';
...
comment on table public.admin_users is '운영자 페이지 접근 권한 테이블. service role로만 조회/관리';
comment on column public.admin_users.email is 'Supabase Auth Google 로그인 이메일. 소문자 저장 권장';
comment on column public.admin_users.role is 'admin은 조회/수동 실행, viewer는 조회만 가능';
...
create index if not exists sync_runs_trigger_type_idx on public.sync_runs (trigger_type);

alter table public.sync_runs add column if not exists trigger_type text not null default 'cron';
alter table public.sync_runs add column if not exists actor_email text;

do $$
begin
  alter table public.sync_runs
  add constraint sync_runs_trigger_type_check check (trigger_type in ('cron', 'manual'));
exception
...
comment on column public.sync_runs.trigger_type is 'cron 자동 실행 또는 운영자 수동 실행 구분';
comment on column public.sync_runs.actor_email is '수동 실행을 요청한 운영자 이메일';
